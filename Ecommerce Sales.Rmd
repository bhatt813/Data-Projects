---
title: "Touch of Modern Assignment"
...
Load the Data into a sqlite database from csv provided. 
```{r}
...
db2 <- dbGetQuery(db, "SELECT b.user_id FROM (SELECT a.user_id, count(a.id) as CT FROM orders a WHERE a.payment_reject = 'False' group by 1) b 
                  WHERE b.CT > 1")
```
Number of users with >1 purchase to validate Second Highest Orders Result (7,459 users with > 1 purchase)

Number of shipping levels
```{r}
dbGetQuery(db, "SELECT Distinct shipping_cost from orders")
```
There are only 4 distinct shipping costs. 

Statistics based on Shipping Levels
```{r}
...
Shipping_Table
```
Based on the order min and max by shipping cost it, the shipping cost is based on item prices.  There is the 10,15 which splits at $115. The 20,25 split also has a similar split at $115 as well, so when we look at costs based on country with US shipping set of (10,15) & UK shipping set of (20,25).

```{r}
cor(Shipping_Table$shipping_cost, Shipping_Table$n)
```
The negative correlation is not in line with economic expectations which adds that the shipping cost may not be influencing user purchase behavior. Though cart abandonment data would provide a much clearer picture. 

To identify trends, I look into day of orders. 
```{r}
...
Month_plot
```
I break out revenue by country. 
```{r}
...
There is definitely some seasonality with March, May, August, October, December peaks and troughs in February, April, June, September, November.  CA has a larger MoM revenue growth rate than the US does bassed on the linear regression model.  

I then look into how frequently users are purchasing in a given year. 
```{r}
order_count_US <- dbGetQuery(db, "SELECT a.user_id, count(a.id) as order_count, sum(item_total) as                               item_rev 
                          ...
                          AND b.country = 'US' 
                          GROUP BY 1")
US_Boxplot <- boxplot(as.numeric(order_count_US$order_count), main="US Orders Box Plot", ylab="Order Count")
```
```{r}
...
order_count_CA <- dbGetQuery(db, "SELECT a.user_id, count(a.id) as order_count, sum(item_total) as item_rev 
                             ...
                             AND b.country = 'CA' 
                             GROUP BY 1")
CA_Boxplot <- boxplot(order_count_CA$order_count, main="CA Orders Box Plot", ylab="Order Count")
```
```{r}
summary(order_count_CA$order_count)
```

A typical customer made 4 purchases in the year, 25% of the users made more than 8 purchases in a year. US users have a higher max number of orders per year by user than CA. 

I then broke down and pivoted by gender and country. 
```{r}
...
gender
```
A typical male has higher REV than a typical female. Females account for higher revenue per order and total order count than males in both countries. Canada has a greater mean revenue per order across genders than US. 

I then pivoted the data by gender, age range, and country. I split the tables so I could see the conclusions a bit easier.
```{r}
...
with most orders coming from male customers in their 30s then age group 40s, and coming from female customers in their 40s then age group 30s. By country grouping shows the same trend.

I then wanted to look at the relationship between purchases (revenue and order count) and days on site. 
```{r}
...
```
As would be expected, the more days on site the more revenue and orders made by the user. 
Based on correlation which is close to 1 for days on site and order_count and revenue by user.  With revenue count and days on site having the stronger relationship. 

I wanted to see if there was any variation between days on site when broken down by gender. 
...
onsite_table
```
On average males have spent more days on the site than females. The typical male or female spent 8 days on the site in 2016.

I also wanted to break down days on site by Age Range. 
```{r}
...
onsite_table_age
```
When looking at days on site by Age typical male customer in their 30s and 50s spend an extra day on the site (9 days) versus all other groups (8 days).
